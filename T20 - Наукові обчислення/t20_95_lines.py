...
class Lines:
    ...
    def _make_lists(self, rows, cols):
        """
        Будує список усіх списків індексів горизонталей,
        вертикалей та діагоналей таблиці
        :param rows: кількість рядків
        :param cols: кількість стовпчиків
        :return: список усіх списків
        """
        ...
        # список усіх списків
        return hors + verts + diags_plusminus + diags_minusplus

    def _clear_one_list(self, grid, lst):
        """
        Підготувати спиок координат кульок для очистки для 1 списку
        :param grid: таблиця
        :param lst: список
        :return: список координат для очистки, додатковий рахунок
        """
        to_clear_one = []
        addscore_one = 0
        k = 0
        ...
        while k <= len(lst) - 5:
            ...
            if bo:
                ...
                while k + i < len(lst):
                    row, col = lst[k + i]
                    bo = grid[row][col]
                    if not bo or bo.fill != color:
                        break

                    candidate.append((row, col))
                    i += 1
                if len(candidate) >= 5:
                    to_clear_one += candidate  # додати до списку на видалення
                    addscore_one += scores[len(candidate)]  # додати рахунок
                k += i
            else:
                k += 1
        return to_clear_one, addscore_one
    ...
    def clear(self, grid):
        ...
        # список усіх списків
        all_lists = self._make_lists(rows, cols)
        for lst in all_lists:
            to_clear_one, addscore_one = self._clear_one_list(grid, lst)
            to_clear += to_clear_one # додати до списку на видалення
            addscore += addscore_one # додати рахунок
        ...
        if (endrow, endcol) in s1:
            ...
            if ok:
                for (i,j) in self.tries:
                    ...
                    row, col = r + i, c + j # робимо крок
                    if (row, col) in s1:
                        # шлях будується від кінцевої позиції
                        path.insert(0, (row, col))
                        break

                else: # цикл не закінчився break
                    raise ValueError(
                        f'Помилка: попередню позицію для {path[0]} '
                        f'не знайдено у множині {s1}')
        return ok
