...
from T14.t14_10_wordguess import Guesser, makeword
...
class WordGuessServer(socketserver.ThreadingTCPServer):
    ...
    def __init__(self, server_address, RequestHandlerClass, filename):
        ...
        socketserver.ThreadingTCPServer.__init__(self, server_address,
                                                    RequestHandlerClass)
        self.glist = deque()        # кільцевий список гравців (відгадувачів)
                                    # типу NetGuesser
...
class RequestHandler(socketserver.StreamRequestHandler):
    ...
    def finish(self):
        ...
        if self.name:
            ...
            for i in range(len(self.server.glist)):
                netguesser = self.server.glist[0]
                if netguesser.getname() == self.name:
                    self.server.glist.popleft()
                    self.server.num_guessers -= 1
                else:
                    self.server.glist.rotate(-1)
        # якщо гравців не залишилось, поновити готовність до гри
        if len(self.server.glist) == 0:
    ...
    def addGuesser(self, name):
        ...
        self.server.glist.append(NetGuesser(self.name, self.wfile))
        self.server.num_guessers += 1
    ...
    def startGame(self):
        ...
        word, guessed = makeword(self.server.filename)
#        print('word', word, 'guessed', guessed)
    ...
    def _processResults(self, points):
        ...
        netguesser = self.server.glist[0] # поточний гравець
...
if __name__ == '__main__':
    ...
    # запустити сервер
    WordGuessServer((HOST, PORT), RequestHandler, 'text.txt').serve_forever()
